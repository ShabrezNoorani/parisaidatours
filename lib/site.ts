export const siteConfig = {
  name: 'Paris Aida Tours',
  tagline: 'Discover Paris Through the Eyes of a Local Expert',
  description:
    'Private guided tours of Paris led by Aida, a state-licensed guide with 8+ years of experience. Notre-Dame, Louvre, Montmartre, Versailles and more.',
  url: 'https://parisaidatours.com',

  contact: {
    email: 'admin@parisaidatours.com',
    phone: '+33 7 45 56 27 18',
    phoneDisplay: '+33 7 45 56 27 18',
    whatsapp: 'https://wa.me/33745562718',
  },

  social: {
    instagram: 'https://www.instagram.com/paris.aida.tours/',
    facebook: 'https://www.facebook.com/parisaidatours/',
    twitter: 'https://x.com/AidaHammami',
    linkedin: 'https://www.linkedin.com/in/a%C3%AFda-douja-hammami/',
    tiktok: 'https://www.tiktok.com/@parisaidatours',
    google: 'https://share.google/V8CpxKpMXodDvD53m',
  },

  bokun: {
    channelUUID: 'ee56c558-0caf-4c80-9e1f-c65eafa073e2',
    loaderScript:
      'https://widgets.bokun.io/assets/javascripts/apps/build/BokunWidgetsLoader.js',
  },

  calendly: {
    baseUrl: 'https://calendly.com/admin-parisaidatours/30min',
  },
};
